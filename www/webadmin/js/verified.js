...
function getDesigner() {
    $.ajax({
        type: "get",
        url: "http://www.houxiaopang.com/api/v1.2/verified/selectall",
        headers: {"Authorization": "Token " + window.localStorage.token},
        success(data){
            data = JSON.parse(data);
            if (data.code === 0) {
                var applyer = data.data.verifies;
                if (applyer.length === 0) {
                    alert("无等待实名认证审核的设计师");
                    return;
                }
                ...
                applyer.forEach(function (item, index) {
                    ...
                    var stroperate = `
        			<tr><td><div><span onclick="pass(${item.userId},${index})" class="colorSpan">通过</span> <span class="colorSpan"
                    onclick="fail(${item.userId},${index})">拒绝</span></div></td></tr>`;
                    ...
                    var strcontent = `
        					<tr>
                                    ...
                                    <td><div>
                                            <span onclick="jump(event)" data-img="${item.cardImgUp}" style="cursor: pointer;">查看</span>
                                        </div></td>
                                    <td><div>
                                            <span onclick="jump(event)" data-img="${item.cardImgDowm}" style="cursor: pointer;">查看</span>
                                        </div></td>                                      
                                    <td><div>
...
function pass(id, index) {
    $.ajax({
        url: "http://www.houxiaopang.com/api/v1.2/verified/updatastatus",
        type: "post",
        headers: {"Authorization": "Token " + window.localStorage.token},
        data: {
            'userId': id,
...
function jump(e) {
    ...
    var model = document.getElementById("modelimg")
    model.children[0].src = "data:image/jpeg;base64," + img;
