<template type="text/template" id="template_pagination">
  <section class="pages-wrap" v-show="pageInfo.total>pageInfo.pagenum">
    <ul class="pagination clearfix">
      ...
      <li :class="{'disabled': pageInfo.current == 1}"><a href="javascript:;"
                                                          @click="clickCurrent(pageInfo.current - 1)"> 上一页 </a></li>
      ...
      <li :class="{'disabled': pageInfo.current == page}"><a href="javascript:;"
                                                                      @click="clickCurrent(pageInfo.current + 1)">
        下一页</a></li>
      <li :class="{'disabled': pageInfo.current == page}"><a href="javascript:;"
                                                                      @click="clickCurrent(page)"> 尾页 </a></li>
    </ul>
  </section>
</template>
<script>
  ...
  export default {
    ...
    computed: {
      ...
      setList: function () {
        var len = this.page, temp = [], list = [], count = Math.floor(this.pageInfo.pagegroup / 2),
